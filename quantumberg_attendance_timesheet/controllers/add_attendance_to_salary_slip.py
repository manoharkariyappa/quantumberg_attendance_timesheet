...
def get_employee_leave_hours(employee, start_date, end_date):
	...
	for leave in leave_applications:
		leave_days = (
			frappe.utils.date_diff(min(leave["to_date"], end_date), max(leave["from_date"], start_date)) + 1
		)
...
def get_holiday_dates(employee):
	...
	if holiday_list:
		return frappe.db.get_all("Holiday", filters={"parent": holiday_list}, pluck="holiday_date")
	return None
